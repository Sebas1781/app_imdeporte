...
@section('page-title', 'Agregar – Cultura Física')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.cultura-fisica.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="tipo" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
                    <option value="actividad" {{ old('tipo', $tipo) === 'actividad' ? 'selected' : '' }}>Actividad</option>
                    <option value="servicio"  {{ old('tipo', $tipo) === 'servicio'  ? 'selected' : '' }}>Servicio de Salud</option>
                </select>
