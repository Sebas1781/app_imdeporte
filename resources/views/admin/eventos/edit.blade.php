...
@section('content')
<div class="max-w-2xl">
    <a href="{{ route('admin.eventos.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-[#7B2D8E] mb-6 transition">
        <i class="fas fa-arrow-left"></i> Volver al listado
    </a>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.eventos.update', $evento) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">{{ old('descripcion', $evento->descripcion) }}</textarea>
            ...
            <div>
                ...
                @if($evento->imagen)
                    <div class="mb-2">
                        <img src="{{ $evento->imagen }}" alt="{{ $evento->titulo }}" class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="url" id="url_externa" name="url_externa" value="{{ old('url_externa', $evento->url_externa) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition"
                       placeholder="https://...">
            ...
            <div>
                ...
                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', $evento->fecha?->format('Y-m-d')) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
            ...
            <div>
                ...
                <input type="number" id="orden" name="orden" value="{{ old('orden', $evento->orden) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
                @error('orden') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" id="activo" name="activo" value="1" {{ $evento->activo ? 'checked' : '' }}
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.eventos.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2.5 px-6 rounded-lg text-sm transition">
                    Cancelar
                </a>
            </div>
        </form>
