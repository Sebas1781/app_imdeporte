...
@section('page-title', 'Editar Convocatoria')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.convocatorias.update', $convocatoria) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="titulo" class="block text-sm font-semibold text-gray-700 mb-1">Título *</label>
                <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $convocatoria->titulo) }}" required
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">{{ old('descripcion', $convocatoria->descripcion) }}</textarea>
            ...
            <div>
                ...
                @if($convocatoria->imagen)
                    <div class="mb-2">
                        <img src="{{ $convocatoria->imagen }}" alt="{{ $convocatoria->titulo }}" class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="url" id="url_externa" name="url_externa" value="{{ old('url_externa', $convocatoria->url_externa) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition"
                       placeholder="https://...">
            ...
            <div>
                ...
                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', $convocatoria->fecha->format('Y-m-d')) }}" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
                @error('fecha') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" id="activo" name="activo" value="1" {{ $convocatoria->activo ? 'checked' : '' }}
