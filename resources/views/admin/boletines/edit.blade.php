...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.boletines.update', $boletin) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="titulo" class="block text-sm font-semibold text-gray-700 mb-1">Título *</label>
                <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $boletin->titulo) }}" required
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">{{ old('descripcion', $boletin->descripcion) }}</textarea>
            ...
            <div>
                ...
                @if($boletin->imagen)
                    <div class="mb-2">
                        <img src="{{ $boletin->imagen }}" alt="{{ $boletin->titulo }}" class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="url" id="url_externa" name="url_externa" value="{{ old('url_externa', $boletin->url_externa) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition"
                       placeholder="https://...">
            ...
            <div>
                ...
                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', $boletin->fecha->format('Y-m-d')) }}" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
                @error('fecha') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" id="activo" name="activo" value="1" {{ $boletin->activo ? 'checked' : '' }}
