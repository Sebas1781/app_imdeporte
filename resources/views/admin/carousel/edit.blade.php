...
@section('page-title', 'Editar Imagen del Carrusel')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.carousel.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                @if($item->imagen)
                    <div class="mb-2">
                        <img src="{{ $item->imagen }}" alt="{{ $item->titulo }}" class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                @endif
