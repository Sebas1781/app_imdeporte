...
@section('content')
<div class="max-w-2xl">
    <a href="{{ route('admin.programas.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-[#7B2D8E] mb-6 transition">
        <i class="fas fa-arrow-left"></i> Volver al listado
    </a>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.programas.update', $programa) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="titulo" class="block text-sm font-semibold text-gray-700 mb-1">Título *</label>
                <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $programa->titulo) }}" required
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">{{ old('descripcion', $programa->descripcion) }}</textarea>
            ...
            <div>
                ...
                @if($programa->imagen)
                    <div class="mb-2">
                        <img src="{{ $programa->imagen }}" alt="{{ $programa->titulo }}" class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="url" id="url_externa" name="url_externa" value="{{ old('url_externa', $programa->url_externa) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition"
                       placeholder="https://...">
            ...
            <div>
                <label for="fecha" class="block text-sm font-semibold text-gray-700 mb-1">Fecha (opcional)</label>
                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', $programa->fecha?->format('Y-m-d')) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
            ...
            <div>
                ...
                <input type="number" id="orden" name="orden" value="{{ old('orden', $programa->orden) }}"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#7B2D8E] focus:border-[#7B2D8E] transition">
                @error('orden') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" id="activo" name="activo" value="1" {{ $programa->activo ? 'checked' : '' }}
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.programas.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-2.5 px-6 rounded-lg text-sm transition">
                    Cancelar
                </a>
            </div>
        </form>
