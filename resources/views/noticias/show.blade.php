...
<section class="py-10 bg-white">
    <div class="max-w-4xl mx-auto px-6">
        <a href="{{ route('noticias.index') }}" class="inline-flex items-center gap-1 text-sm text-[#7B2D8E] hover:text-[#5c1a6e] mb-6 transition">
        ...
        <span class="text-xs font-bold text-[#7B2D8E] bg-[#f3e8f7] px-3 py-1 rounded">{{ $noticia->fecha->translatedFormat('d F, Y') }}</span>
        ...
        @if($recientes->count())
            <hr class="my-10">
            <h3 class="text-xl font-bold text-gray-800 mb-6">Otras noticias</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach($recientes as $item)
                    <a href="{{ route('noticias.show', $item) }}" class="bg-white rounded-xl shadow-lg overflow-hidden group border border-gray-100 hover:shadow-xl transition">
