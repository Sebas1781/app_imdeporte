...
<section class="py-10 bg-white">
    <div class="max-w-6xl mx-auto px-4">
        <div class="relative">
            <div id="eventos-container" class="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                    ...
                    <a href="{{ route('eventos.show', $evento) }}" {{ $evento->url_externa ? 'target=_blank' : '' }}
                       class="bg-white rounded-xl shadow-lg overflow-hidden group border border-gray-100 evento-card hover:shadow-xl transition" style="display: none;">
                        ...
                        <div class="p-2 sm:p-4 md:p-5">
                            @if($evento->fecha)
                                <span class="text-[10px] sm:text-xs font-bold text-[#7B2D8E] bg-[#f3e8f7] px-2 py-0.5 rounded">{{ $evento->fecha->translatedFormat('d \d\e F') }}</span>
                            @endif
                            <h4 class="text-gray-800 font-bold mt-2 text-xs sm:text-sm leading-snug">{{ $evento->titulo }}</h4>
                            <p class="hidden sm:block text-gray-500 text-xs mt-2">{{ Str::limit($evento->descripcion, 100) }}</p>
                        </div>
                    </a>
                @empty
            ...
            <button id="eventos-prev" class="absolute left-2 md:left-0 top-1/2 -translate-y-1/2 md:-translate-x-4 w-8 h-8 md:w-10 md:h-10 bg-[#7B2D8E] hover:bg-[#5c1a6e] shadow-lg rounded-full flex items-center justify-center text-white transition z-10">
                <i class="fas fa-chevron-left text-sm md:text-base"></i>
            </button>
            <button id="eventos-next" class="absolute right-2 md:right-0 top-1/2 -translate-y-1/2 md:translate-x-4 w-8 h-8 md:w-10 md:h-10 bg-[#7B2D8E] hover:bg-[#5c1a6e] shadow-lg rounded-full flex items-center justify-center text-white transition z-10">
